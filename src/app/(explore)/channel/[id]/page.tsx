...
export default function ChannelDetailPage() {
  ...
  const { user } = useAuth()
  const [channelCampaigns, setChannelCampaigns] = useState<Campaign[]>([])
  ...
  useEffect(() => {
    const fetchChannelCampaigns = async () => {
      try {
        setIsLoading(true)
        setError(null)

        const userId = user?.id
        console.log('📡 Fetching campaigns for channel:', channelId, 'userId:', userId)
        ...
        if (response.success && response.data) {
          ...
          // Filter campaigns by channel (checking tags/categories)
          const filtered = allCampaigns
            ...
            .filter((campaign) => {
              ...
              return campaign.tags.some((tag: string) => 
                tag.toLowerCase().includes(channelId.toLowerCase())
              )
            })

          setChannelCampaigns(filtered)
          console.log(`✅ Loaded ${filtered.length} campaigns for channel: ${channelName}`)
        } else {
          setError(response.message || 'Failed to load campaigns')
        }
      } catch (err: any) {
        const appError = ErrorHandler.handleApiError(err)
        ErrorHandler.logError(appError, 'Channel Detail Page')
            ...
            <>
              ...
              {channelCampaigns.length > 0 ? (
                ...
                <div className="text-center py-20">
                  <p className="text-white/60 text-lg">No campaigns found for this channel.</p>
                </div>
              )}
            </>
          )}
