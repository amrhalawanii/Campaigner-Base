...
export default function IndustryDetailPage() {
  ...
  const { user } = useAuth()
  const [industryCampaigns, setIndustryCampaigns] = useState<Campaign[]>([])
  ...
  useEffect(() => {
    const fetchIndustryCampaigns = async () => {
      try {
        setIsLoading(true)
        setError(null)

        const userId = user?.id
        console.log('📡 Fetching campaigns for industry:', industryId, 'userId:', userId)
        ...
        if (response.success && response.data) {
          ...
          // Filter campaigns by industry (checking tags/categories)
          const filtered = allCampaigns
            ...
            .filter((campaign) => {
              ...
              return campaign.tags.some((tag: string) => 
                tag.toLowerCase().includes(industryId.toLowerCase())
              )
            })

          setIndustryCampaigns(filtered)
          console.log(`✅ Loaded ${filtered.length} campaigns for industry: ${industryName}`)
        } else {
          setError(response.message || 'Failed to load campaigns')
        }
      } catch (err: any) {
        const appError = ErrorHandler.handleApiError(err)
        ErrorHandler.logError(appError, 'Industry Detail Page')
    ...
    if (industryId) {
      fetchIndustryCampaigns()
    }
  }, [industryId, industryName, user])
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#171a00] to-black text-white">
      ...
      <main className="pt-32 pb-12">
        <div className="container mx-auto px-6">
          <button
            onClick={() => router.back()}
            className="inline-flex items-center gap-2 text-sm text-[#cced00] hover:text-[#d6ff2f] transition-colors mb-6"
          >
          ...
          {isLoading ? (
            ...
            <>
              <div className="mb-12">
                <h1 className="text-4xl md:text-5xl font-bold mb-4 uppercase">{industryName}</h1>
                <div className="text-sm text-white/60">
                  {industryCampaigns.length} {industryCampaigns.length === 1 ? "campaign" : "campaigns"}
                </div>
              </div>
              ...
              {industryCampaigns.length > 0 ? (
                ...
                <div className="text-center py-20">
                  <p className="text-white/60 text-lg">No campaigns found for this industry.</p>
                </div>
              )}
            </>
          )}
