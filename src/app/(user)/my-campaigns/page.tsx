  ...
  useEffect(() => {
    const fetchCampaigns = async () => {
      try {
        ...
        console.log('📡 Fetching my campaigns, userId:', userId)

        // Get home page data to extract my_campaigns
        ...
        if (homePageResponse.success && homePageResponse.data) {
          ...
          // Extract campaigns from my_campaigns collection
          if (data.my_campaigns) {
            if (Array.isArray(data.my_campaigns)) {
              myCampaignsData = data.my_campaigns.map(transformCampaign)
            } else if (typeof data.my_campaigns === 'object' && data.my_campaigns.items && Array.isArray(data.my_campaigns.items)) {
              myCampaignsData = data.my_campaigns.items.map(transformCampaign)
            }
          }

          // If no my_campaigns found, try to get all campaigns as fallback
          if (myCampaignsData.length === 0) {
            console.log('ℹ️ No my_campaigns found, fetching all campaigns as fallback')
            ...
            if (allCampaignsResponse.success && allCampaignsResponse.data) {
              ...
              myCampaignsData = allCampaigns.map(transformCampaign)
            }
          }
        ...
        ErrorHandler.logError(appError, 'My Campaigns Page')
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#171a00] to-black text-white">
      ...
      <main className="pt-32 pb-16">
        <div className="container mx-auto px-6 max-w-7xl">
          <Link
            href="/"
            className="inline-flex items-center gap-2 text-sm font-medium text-[#cced00] hover:text-[#d6ff2f] transition-colors mb-8"
          >
