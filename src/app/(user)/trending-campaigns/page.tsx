  ...
  useEffect(() => {
    const fetchCampaigns = async () => {
      try {
        ...
        console.log('📡 Fetching trending campaigns, userId:', userId)

        // Get home page data to extract trending_campaigns
        ...
        if (homePageResponse.success && homePageResponse.data) {
          ...
          // Extract campaigns from trending_campaigns collection
          if (data.trending_campaigns) {
            if (Array.isArray(data.trending_campaigns)) {
              trendingData = data.trending_campaigns.map(transformCampaign)
            } else if (typeof data.trending_campaigns === 'object' && data.trending_campaigns.items && Array.isArray(data.trending_campaigns.items)) {
              trendingData = data.trending_campaigns.items.map(transformCampaign)
            }
          }

          // If no trending_campaigns found, try to get all campaigns as fallback
          if (trendingData.length === 0) {
            console.log('ℹ️ No trending_campaigns found, fetching all campaigns as fallback')
            ...
            if (allCampaignsResponse.success && allCampaignsResponse.data) {
              ...
              trendingData = allCampaigns.map(transformCampaign)
            }
          }

          setCampaigns(trendingData)
        ...
        ErrorHandler.logError(appError, 'Trending Campaigns Page')
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#171a00] to-black text-white">
      ...
      <main className="pt-24 pb-12">
        <div className="container mx-auto px-6">
          ...
          <h1 className="text-4xl font-bold mb-12 uppercase tracking-tight">Trending Campaigns</h1>
